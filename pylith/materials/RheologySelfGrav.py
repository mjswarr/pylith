...
from .materials import RheologySelfGravElasticity as ModuleRheology


class RheologySelfGravElasticity(PetscComponent, ModuleRheology):
    """
    Abstract base class for bulk rheology of self gravitating elastic material.
    """
    ...
    def __init__(self, name="rheologyselfgravelasticity"):
        """Constructor.
        """
        PetscComponent.__init__(self, name, facility="rheologyselfgravelasticity")

    def preinitialize(self, problem):
        from pylith.mpi.Communicator import mpi_is_root
        if mpi_is_root():
            self._info.log("Performing minimal initialization of self gravitating elasticity rheology '%s'." %
                           self.aliases[-1])
